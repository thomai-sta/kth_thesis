...
for svmf in SVM_folds:
    ...
    plt.plot(n_components, dor[svmf], label="DOR",
             linewidth=2.0, color=colors2[idx])
    idx += 1
    plt.plot(n_components, dor_3[svmf], label="DOR 3 FU",
             linewidth=2.0, color=colors2[idx])
    idx += 1
    plt.plot(n_components, dor_CV[svmf], label="DOR with CV",
             linewidth=2.0, color=colors2[idx])
    idx += 1
    plt.plot(n_components, dor_CV_3[svmf],
             label="DOR with CV 3 FU", linewidth=2.0,
             color=colors2[idx])
    plt.legend(loc=0)
    plt.grid()
    plt.xlabel('Number of States')
